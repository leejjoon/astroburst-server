...
/// Forward of `unshuffle_bytes`: take natural [pixel][byte] order and byte-
/// "plane" it (all pixels' byte 0, then all byte 1, …). This is the transpose
/// GZIP_2 applies before gzipping so a smooth image's low-entropy high bytes
/// (exponents/signs) sit contiguously and gzip finds long runs.
...
/// Encode raw big-endian pixel bytes as a GZIP_2 tile: byte-shuffle (when
/// `bytepix > 1`) then gzip. The exact inverse of `gzip_decode(.., true)`, and
/// the same RFC 1952 gzip container / `Compression::default()` level the
/// single-image writer already uses for its raw-row gzip fallback.
...
mod tests {
    ...
    fn gzip2_encode_roundtrips_through_decoder() {
        // Raw big-endian f32 bytes → gzip2_encode → gzip_decode(is_gzip2) == input.
        ...
        // bytepix == 1 path (no shuffle) also round-trips.
    ...
    fn gzip2_encode_is_deterministic() {
        // Determinism underpins cross-request Range resume of a compressed pull.
