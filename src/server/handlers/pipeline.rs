...
pub struct PipelineParams {
    ...
    /// Prefix for result slot keys, e.g. `"pipe/"` → `"pipe/Ha"`, `"pipe/OIII"`.
    /// Defaults to empty string so labels are used as-is.
...
/// Masters (bias, dark, flat) are built from the supplied calibration frames.
/// Each stacked channel is stored in the session cache under `{prefix}{label}`.
///
/// Returns 202 immediately.  Subscribe to `GET /sessions/:sid/jobs/:jid/stream`
/// for SSE progress (`progress`, `complete`, `error`).
pub async fn run(
    ...
    // Compute slot names before moving params into the closure.
    ...
    let slot_names_resp = slot_names.clone(); // kept for the 202 response
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // Dark exposure scaling: only meaningful when bias removes pedestal.
        ...
        // ── store each channel in the session cache ───────────────────────
