...
pub struct StackParams {
    ...
    /// Session cache key for the output image. Defaults to `"stacked"`.
...
pub struct DrizzleParams {
    pub paths: Vec<String>,
    /// Session cache key for the output image. Defaults to `"drizzled"`.
    pub result_slot: Option<String>,
    pub scale: Option<f64>,
    pub pixfrac: Option<f64>,
    /// `"square"` (default), `"gaussian"`, `"lanczos3"`
...
///
/// Sigma-clip stack a list of FITS files and store the result in the session
/// image cache.  Returns 202 immediately with a `job_id`; poll
/// `GET /sessions/:sid/jobs/:jid` or subscribe to
/// `GET /sessions/:sid/jobs/:jid/stream` for progress.
///
/// The result image is available under the `slot` key after the job completes.
pub async fn stack(
...
///
/// Drizzle-combine a list of FITS files into the session cache.
/// Same async job pattern as `/stacking/stack`.
