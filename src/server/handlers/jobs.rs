...
///
/// Returns the current status, progress percentage, and timestamps for a job.
/// Designed for polling fallback when SSE is not available.
...
/// The cancellation signal is delivered via `CancellationToken`; the spawned
/// task is responsible for checking it and calling `job.set_cancelled()`.
...
///
/// SSE stream for real-time job progress.  Each event carries a `type` field:
/// `progress` (with `pct` and `stage`), `complete`, or `error`.
///
/// Returns 409 if a subscriber is already connected — only one SSE consumer
/// per job is supported to keep backpressure simple.
