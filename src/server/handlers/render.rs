...
/// Encode an 8-bit grayscale pixel buffer as PNG bytes (in memory).
...
///
/// Apply explicit STF parameters to a cached image and return PNG bytes.
pub async fn render(
...
///
/// Auto-compute STF parameters from image stats and return PNG bytes along
/// with the computed STF triple so the caller can re-use them.
pub async fn auto_render(
    ...
    // Encode STF params as a header so the caller sees them without a second round-trip.
...
///
/// Render a rectangular crop of a cached image. STF parameters are optional;
/// when absent, auto-STF is computed from the *crop* statistics.
