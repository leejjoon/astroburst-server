...
pub struct OpenParams {
    pub path: String,
    /// Key under which the image is stored in session.cache. Defaults to the path.
...
///
/// Load a FITS/ASDF file into the session image cache.
/// Returns image dimensions, per-channel stats, auto-STF parameters, and the
/// header key→value map so the caller has everything needed for display.
...
///
/// Return the full categorised header for a slot already loaded in the cache.
/// If the slot exists but was loaded without a header, this endpoint upgrades it.
/// 404 when the slot is not in the cache at all.
pub async fn fits_header(
    ...
    // If we already have the header, use it. Otherwise fail fast — caller
    // should have used fits/open with the full path first.
    let header = entry
        .header()
