//! v2 render handler — `POST /v2/sessions/:sid/render`.
//!
//! The agent-facing render endpoint and the highest-volume call in the API
//! (`astro-image-api.md` §4). It wires together the three independent pieces
//! built in earlier slices — zscale limits (#10), colormap LUTs + RGB8 PNG
//! encoder (#11), the pointwise stretch vocabulary (#12) — plus new dispatch
//! and response-shaping logic:
//!
//! - **Scale resolution**: `zscale` (auto), `minmax` (region min/max),
//!   `manual` (caller `vmin`/`vmax`), `percentile` (via the `/stats`
//!   [`percentile`] helper).
//! - **Stretch**: dispatches to the five [`StretchKind`] curves.
//! - **Colormap**: `gray`/`viridis`, encoded through [`encode_png_rgb8`].
//! - **`max_dim` binning**: `factor = ceil(max(w, h) / max_dim)`, applied via
//!   [`area_downsample`] (the same primitive `/bin` uses); skipped when the
//!   region already fits.
//! - **Overlays**: `crosshair` (pixel or sky coords) and `scalebar`
//!   (`length_arcsec`), drawn on the *final*, post-binning buffer. A `scalebar`
//!   on a WCS-less image is silently omitted, not an error.
//! - **Region handling**: unlike `/stats`/`/histogram`, a region request
//!   **always clamps silently** to the on-image intersection (via
//!   [`resolve_region_clamped`]) — panning near an edge shows what is there
//!   rather than erroring.
//! - **Statelessness**: never creates a derived `image_ref` and never changes
//!   the session's `active_ref` — a pure read.
//!
//! The body is raw PNG bytes (RGB8); the fully-resolved parameters are echoed
//! in an `x-render-resolved` JSON response header so the agent can reason
//! numerically about what it just saw and reproduce/tweak the render.
...
pub struct RenderParams {
    ...
    /// Optional region; when omitted, the full frame is rendered.
    #[serde(default)]
    pub region: Option<RegionSpec>,
    /// Scale/stretch parameters. Defaults to zscale + linear.
    #[serde(default)]
    pub scale: Option<ScaleSpec>,
    /// Colormap name. Defaults to `gray`.
    #[serde(default)]
    pub colormap: Option<String>,
    /// Invert the colormap (black↔white / low↔high). Defaults to false.
    #[serde(default)]
    pub invert_cmap: bool,
    /// Long side of the PNG; the region is binned so `max(w, h) <= max_dim`.
    /// Omitted / null means no downsampling.
    #[serde(default)]
    pub max_dim: Option<usize>,
    /// Optional overlays (`crosshair`, `scalebar`). Unknown types are ignored.
    #[serde(default)]
...
pub struct ScaleSpec {
    /// `zscale` | `minmax` | `percentile` | `manual`. Defaults to `zscale`.
    #[serde(default)]
    pub algorithm: Option<String>,
    /// `linear` | `log` | `sqrt` | `asinh` | `power`. Defaults to `linear`.
    #[serde(default)]
    pub stretch: Option<String>,
    /// Used when `algorithm = manual`.
    #[serde(default)]
    pub vmin: Option<f64>,
    #[serde(default)]
    pub vmax: Option<f64>,
    /// `[lo, hi]` on a 0–100 scale; used when `algorithm = percentile`.
    #[serde(default, alias = "percentiles")]
    pub percentile: Option<[f64; 2]>,
    /// Softening length for the `asinh` stretch.
    #[serde(default)]
    pub asinh_a: Option<f64>,
    /// Exponent for the `power` stretch.
    #[serde(default)]
    pub power: Option<f64>,
    /// Contrast for the `zscale` algorithm.
    #[serde(default)]
...
/// A draw operation on the final display buffer, in *original image* pixel
/// coordinates (converted to display coordinates inside the blocking stage).
enum DrawOp {
    /// A full-width/full-height crosshair centered on `(px, py)`.
    Crosshair { px: f64, py: f64 },
    /// A horizontal scalebar `length_display_px` long in the final buffer.
    Scalebar { length_display_px: f64 },
}

const CROSSHAIR_COLOR: [u8; 3] = [255, 0, 0]; // red
const SCALEBAR_COLOR: [u8; 3] = [255, 255, 255]; // white

/// Build the list of draw ops from the request's overlays, resolving sky
/// coordinates against the WCS and dropping anything unrenderable (unknown
/// types, a scalebar without a WCS, a crosshair with no usable coordinates).
fn build_overlays(
    ...
    for ov in overlays {
        ...
        match kind {
            "crosshair" => {
                // Pixel coords take precedence; else project sky coords.
...
/// Draw the overlays onto the RGB8 display buffer (`w`×`h`).
fn draw_overlays(
    buf: &mut [u8],
    w: usize,
    h: usize,
    ...
    for op in ops {
        match *op {
            DrawOp::Crosshair { px, py } => {
                ...
                // Vertical line down the whole buffer at column dx.
                for y in 0..h as i64 {
                    set_px(buf, w, h, dx, y, CROSSHAIR_COLOR);
                }
                // Horizontal line across the whole buffer at row dy.
                for x in 0..w as i64 {
                    set_px(buf, w, h, x, dy, CROSSHAIR_COLOR);
                }
            }
            DrawOp::Scalebar { length_display_px } => {
                // Bottom-left, inset a few pixels; a short 3px-thick bar.
...
/// Resolve `(vmin, vmax)` for the display buffer per the requested algorithm.
fn resolve_scale(
    display: &Array2<f32>,
    algorithm: &str,
    scale: &ScaleSpec,
    ...
    match algorithm {
        "zscale" => {
            ...
            // All-NaN → zscale returns NaN; fall back to a degenerate 0..1.
        ...
        "manual" => {
            // Passthrough; fall back to region min/max for a missing bound.
...
/// Finite-only `(min, max)` of an array; `(0, 1)` if there are no finite pixels.
...
pub async fn render(
    ...
    // Region: always clamp silently to the on-image intersection.
    ...
    // Materialize the region as an owned, standard-layout f32 array.
    let region_arr = arr
        .slice(s![resolved.y..resolved.y + resolved.height, resolved.x..resolved.x + resolved.width])
        .to_owned();

    // Binning factor from max_dim.
    ...
    // Parse scale / stretch / colormap up front so bad params fail fast (400).
    ...
    // Everything below is CPU-bound; run it off the async runtime.
    ...
    let (png, vmin, vmax, below_frac, above_frac) = tokio::task::spawn_blocking(move || {
        // 1. Downsample to the display buffer (if binning applies).
        ...
        // 2. Resolve scale limits on the displayed buffer.
        let (vmin, vmax) = resolve_scale(&display, &alg_c, &scale)?;
        let range = vmax - vmin;

        // 3. Normalize → stretch, tracking clipped fractions over valid pixels.
        let data = display
            .as_slice()
        ...
        for &v in data {
            if v.is_finite() {
                ...
                // NaN → colormap maps to index 0 (black); preserved through invert.
                norm.push(f32::NAN);
            }
        }

        // 4. Colormap → RGB8, draw overlays, encode PNG.
