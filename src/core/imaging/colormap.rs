//! Colormap lookup tables and the RGB8 PNG encoder for the v2 render path.
//!
//! A render's final stage maps *normalized* pixel values (the `[0, 1]` output of
//! the scale + stretch stages) through a 256-entry RGB lookup table, then encodes
//! the resulting RGB8 buffer as a PNG. Every v2 render — grayscale included — goes
//! through this single RGB8 path so the render handler has one uniform encoder;
//! v1's `encode_png_l8` is left untouched for v1's own endpoints.
//!
//! Only `Gray` and `Viridis` are implemented here (issue #11). The remaining
//! colormaps from `astro-image-api.md` (`heat`/`inferno`/`plasma`/`cool`/
//! `rainbow`/`bone`) are follow-ups: each is "just another baked LUT" once this
//! path exists.

use image::{codecs::png::PngEncoder, ColorType, ImageEncoder};

/// Colormaps supported by the v2 render path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Colormap {
    /// Identity grayscale: `r = g = b = round(v * 255)`.
    Gray,
    /// matplotlib's perceptually-uniform `viridis`.
    Viridis,
}

impl Colormap {
    /// Parse a colormap by its API name (as it appears in the render request's
    /// `colormap` field). Unknown names return an `Err` carrying a
    /// human-readable message listing the supported names — the render handler
    /// maps this to a `bad_request` rather than panicking.
    ...
    /// Map a single normalized value in `[0, 1]` to an `[r, g, b]` triple.
    ///
    /// Values are clamped to `[0, 1]` first; non-finite values (NaN) map to the
    /// low end of the map (index 0).
    #[inline]
    fn lookup(self, v: f32) -> [u8; 3] {
        // NaN fails the `> 0.0` comparison and stays at 0.0.
        ...
        let idx = (clamped * 255.0).round() as usize; // 0..=255
        match self {
            Colormap::Gray => {
                let g = idx as u8;
                [g, g, g]
...
/// Apply a colormap to normalized `[0, 1]` values, returning an interleaved
/// RGB8 buffer (`3 * values.len()` bytes: `r, g, b, r, g, b, …`).
...
/// Encode an interleaved RGB8 buffer as PNG bytes (in memory).
///
...
/// 256-entry `viridis` RGB lookup table.
///
/// Baked from a 6th-order polynomial fit to matplotlib's `viridis` (matplotlib
/// was unavailable in the build sandbox to dump the exact table). The fit tracks
/// matplotlib to within a few RGB levels across the whole map — verified at the
/// canonical reference points v ∈ {0, 0.25, 0.5, 0.75, 1.0} (see the tests) —
/// which satisfies issue #11's tolerance. Swap in the exact matplotlib table if
/// tighter fidelity is ever needed.
#[rustfmt::skip]
...
mod tests {
    ...
    fn gray_is_exact_round_of_v_times_255() {
        // Gray must produce r = g = b = round(v * 255) for every input.
        ...
        // Spot-check the trivial anchors.
    ...
    fn values_are_clamped_and_nan_maps_low() {
        // Out-of-range and non-finite values must not panic; they clamp.
        let rgb = apply_colormap(&[-1.0, 2.0, f32::NAN], Colormap::Gray);
        assert_eq!(&rgb[0..3], [0, 0, 0]); // below 0 → 0
        assert_eq!(&rgb[3..6], [255, 255, 255]); // above 1 → 255
        assert_eq!(&rgb[6..9], [0, 0, 0]); // NaN → low end
        // Same clamping for viridis (index stays in bounds, no panic).
    ...
    fn viridis_matches_matplotlib_at_reference_points() {
        // matplotlib/astropy viridis RGB at v = 0, 0.25, 0.5, 0.75, 1.0.
        let refs: [(f32, [u8; 3]); 5] = [
            (0.0, [68, 1, 84]),
            (0.25, [59, 82, 139]),
            (0.5, [33, 145, 140]),
    ...
    fn encode_png_rgb8_round_trips_through_the_decoder() {
        // A tiny 2×2 RGB image round-trips: encode → decode → identical pixels.
        let width = 2;
        let height = 2;
        #[rustfmt::skip]
        let rgb: Vec<u8> = vec![
        ...
        // Decodable by the image crate, dimensions + pixels preserved.
