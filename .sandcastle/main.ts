...
// uses pnpm — pnpm-lock.yaml is the committed lockfile, not package-lock.json).
const hooks = {
  // CI=true tells pnpm it's running non-interactively, so it proceeds with
  // any confirmation-gated action (e.g. rebuilding an existing node_modules
  // dir) instead of aborting with ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY
  // — there is never a TTY in a sandbox. This matters because the planner
  // phase (no branchStrategy => "head" mode) mounts the real repo directory
  // read-write rather than an isolated worktree, so it sees (and may want to
  // rebuild) the host's actual node_modules, not an empty one.
